export default {
  ...
  VWR: { // TODO: Better naming
    ...
    calculateWithdrawal: (results, gains, costs, taxes, withDrawalMin, withDrawalMax) => {
      const initialCapital = results[0]
      const lastCapital = results[results.length - 1]
      if (initialCapital <= lastCapital) { // Don't worry if you have more than enough...
        return withDrawalMax
      }

      const trailing_11 = results.slice(-11, results.length)
      const result_trailing_11 = trailing_11[trailing_11.length - 1] - trailing_11[0]
      const avg_result_trailing_12_min = (result_trailing_11 + gains - costs - taxes - withDrawalMin) / (trailing_11.length + 1)
      if (avg_result_trailing_12_min < withDrawalMin) { // Past 12 months returns are less than minimum? Use minimum.
        return withDrawalMin
      }

      const avg_result_trailing_12_max = (result_trailing_11 + gains - costs - taxes - withDrawalMax) / (trailing_11.length + 1)
